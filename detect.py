...
def det_postproccess(detection):
    """
    Get the coordinates detection bbox with the biggest area and save it
    Coordinates are in such order: ymax, xmin, ymin, xmax
    """

    # no detection case handler
    if len(detection) == 0:
        return None
    ...
    for i in range(len(detection)):
        ymax, xmin, ymin, xmax = detection[i][:4]
        area_i = abs(ymax - ymin) * abs(xmax - xmin)
        if area_i > area:
            area = area_i
            index = i
...
def plot_detections(img, detections, with_keypoints=True):
    ...
    # detections_1ele_area = [np.abs(i[0] * img.shape[0] - i[2] * img.shape[0]) * np.abs(i[1] * img.shape[1] - i[3] * img.shape[1]) for i in detections]
    # biggest_index = detections_1ele_area.index(max(detections_1ele_area))
    ...
    for i in range(detections.shape[0]):
        ...
        rect = patches.Rectangle((xmin, ymin), xmax - xmin, ymax - ymin,
                                 linewidth=10, edgecolor="yellow", facecolor="none",
        ...
        if with_keypoints:
            for k in range(6):
                ...
                circle = patches.Circle((kp_x, kp_y), radius=5, linewidth=3,
                                        color="blue",
                                        alpha=detections[i, 16])
                ax.add_patch(circle)
...
def run_blazeface(img_path='1face.png', weight_path="blazefaceback.pth", anchor_path="anchorsback.npy"):
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    back_net = BlazeFace(back_model=True).to(device)
    back_net.load_weights(weight_path)
    back_net.load_anchors(anchor_path)
    ...
    detections = back_net.predict_on_image(img_tensor)
    output_dets = []
    for i in range(len(detections)):
        if torch.cuda.is_available():
            det_i = (detections.detach().cpu().numpy() * 256).astype(int)
        else:
            det_i = (detections.detach().numpy() * 256).astype(int)
        ymax, xmin, ymin, xmax = det_i[i][2], det_i[i][1], det_i[i][0], det_i[i][3]
        output_dets.append((ymax, xmin, ymin, xmax))

    biggest_index = det_postproccess(output_dets)
    # print('biggest face index:', biggest_index)
    return plot_detections(img_np_rgb, detections[biggest_index: biggest_index+1])
...
if __name__ == '__main__':
    run_blazeface('C:/Users/ziyi.guo/Documents/Github/CommonDatasets/face_dataset0/original/clean/agedb30/7.jpg')
